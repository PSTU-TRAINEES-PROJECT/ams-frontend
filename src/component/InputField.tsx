...
const InputField: React.FC<InputFieldProps> = ({ id, label, type, placeholder, IconComponent, register, error }) => {
  return (
    <div className="relative w-96">
      ...
      {error && <p className="text-error-400 text-sm mt-1">{error.toString()}</p>}
    </div>
  );
};
